...
void moveCards(Pile *source, Pile *destination, Card *cardToMove) {
    if (source == NULL || destination == NULL || cardToMove == NULL) {
        printf("Invalid operation.\n");
        return;
    }

    Card *current = source->firstCard;
    Card *prev = NULL;
    while (current != NULL && current != cardToMove) {
        prev = current;
    ...
    if (prev == NULL) {
        source->firstCard = NULL;
    } else {
        prev->nextCard = NULL;
    }

    if (destination->firstCard == NULL) {
        destination->firstCard = cardToMove;
    } else {
        destination->lastCard->nextCard = cardToMove;
    }

    Card *last = cardToMove;
    while (last->nextCard != NULL) {
        last = last->nextCard;
    }
    destination->lastCard = last;
}
